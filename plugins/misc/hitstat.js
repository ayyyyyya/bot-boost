...
export const run = {
   ...
   async: async (m, {
      ...
      const types = command == 'hitstat' ? statistic : Object.fromEntries(Object.entries(statistic).filter(([_, prop]) => format(prop.lasthit, 'ddMMyy') == format(Date.now(), 'ddMMyy')))
      let stat = Object.keys(types)
      if (stat.length == 0) return client.reply(m.chat, Utils.texted('bold', `🚩 No command used.`), m)
