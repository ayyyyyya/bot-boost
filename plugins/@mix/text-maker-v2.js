export const run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         if (!text1 || !text2) return client.reply(m.chat, Utils.example(isPrefix, command, 'neoxr | bot'), m)
         if (text1.length > 10 || text2.length > 10) return client.reply(m.chat, Utils.texted('bold', `🚩 Text is too long max 10 characters.`), m)
         let old = new Date()
